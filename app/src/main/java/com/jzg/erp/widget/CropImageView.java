...
import com.jzg.erp.widget.FloatDrawable;
...
public class CropImageView extends ImageView {
	...
	public void setDrawable(Drawable mDrawable, int cropWidth, int cropHeight, int titleHeight) {
		...
		case MotionEvent.ACTION_MOVE:
			if (mStatus == STATUS_MULTI_TOUCHING) {
				...
				if (!(dx == 0 && dy == 0)) {
					switch (currentEdge) {
					case EDGE_LT:
						mDrawableFloat.set(mDrawableFloat.left + dx, mDrawableFloat.top + dy, mDrawableFloat.right,
								mDrawableFloat.bottom);
						break;

					case EDGE_RT:
						mDrawableFloat.set(mDrawableFloat.left, mDrawableFloat.top + dy, mDrawableFloat.right + dx,
								mDrawableFloat.bottom);
						break;

					case EDGE_LB:
						mDrawableFloat.set(mDrawableFloat.left + dx, mDrawableFloat.top, mDrawableFloat.right,
								mDrawableFloat.bottom + dy);
						break;

					case EDGE_RB:
						mDrawableFloat.set(mDrawableFloat.left, mDrawableFloat.top, mDrawableFloat.right + dx,
								mDrawableFloat.bottom + dy);
						break;
	...
	protected void checkBounds() {
		...
		if (mDrawableFloat.top < getTop() - mTitleHeight) {
			newTop = getTop() - mTitleHeight;
			isChange = true;
		}
		...
		if (mDrawableFloat.bottom > getBottom() - mTitleHeight) {
			newTop = getBottom() - mDrawableFloat.height() - mTitleHeight;
			isChange = true;
		}
		...
		if(mDrawableFloat.height() <= getHeight()){
			...
			newTop = 0;
			mDrawableFloat.set(newLeft, 0, newLeft + mDrawableFloat.width(), getHeight());
		}
		
		if (mDrawableFloat.width() > getWidth()) {
			mDrawableFloat.set(0, newTop, getWidth(), mDrawableFloat.height());
		}
