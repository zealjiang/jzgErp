        ...
        JzgApp.getApiServer().isUpdate(map)
            ...
            public void onNext(UpdateApp updateApp) {
                ...
                if (updateApp.getStatus() == 1) {
                    mUpdate = updateApp;
                    if(updateApp.getSource()==null)return;
                    if (curVersionCode < updateApp.getSource().getVersionCode()) {
                        apkUrl = updateApp.getSource().getDownloadUrl();
                        updateMsg = updateApp.getSource().getUpdateLog();
                        String upDateForce = updateApp.getSource().getUpdateForce();
...
//                .subscribe(new Action1<UpdateApp>() {
//                    @Override
//                    public void call(UpdateApp updateApp) {
//
//                        //进度条对话框不显示 - 检测结果也不显示
//                        if (mProDialog != null && !mProDialog.isShowing()) {
//                            return;
...
//		}
    ...
    private Runnable mdownApkRunnable = new Runnable() {
        ...
        public void run() {
            try {
                String apkName = "JZGJC_" + mUpdate.getSource().getVersionName() + ".apk";
                String tmpApk = "JZGJC_" + mUpdate.getSource().getVersionName() + ".tmp";
                //判断是否挂载了SD卡
